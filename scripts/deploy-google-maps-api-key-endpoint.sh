...
# Script to deploy the updated Google Maps Supabase Edge Function with the API key endpoint
...
echo -e "${YELLOW}Deploying updated Google Maps Supabase Edge Function...${NC}"
...
echo -e "${YELLOW}Deploying updated Google Maps function...${NC}"
supabase functions deploy google-maps --no-verify-jwt

if [ $? -eq 0 ]; then
    echo -e "${GREEN}Google Maps function deployed successfully!${NC}"
    echo -e "${YELLOW}Testing the API key endpoint...${NC}"
    
    # Get the Supabase project URL
    PROJECT_URL=$(supabase status | grep "API URL" | awk '{print $3}')
    
    if [ -n "$PROJECT_URL" ]; then
        echo -e "${GREEN}Your Google Maps API key endpoint is available at:${NC}"
        echo "$PROJECT_URL/functions/v1/google-maps/api-key"
        echo -e "${YELLOW}You can test it with:${NC}"
        echo "curl -X GET $PROJECT_URL/functions/v1/google-maps/api-key -H \"Authorization: Bearer \$SUPABASE_ANON_KEY\""
    else
